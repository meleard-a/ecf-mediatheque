 ...
 */
final class Version20211105130536 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__books AS SELECT id, author_id, type_id, borrow_id, title, cover, publication_date FROM books');
        $this->addSql('DROP TABLE books');
        $this->addSql('CREATE TABLE books (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, author_id INTEGER NOT NULL, type_id INTEGER NOT NULL, borrow_id INTEGER DEFAULT NULL, title VARCHAR(255) NOT NULL COLLATE BINARY, cover VARCHAR(80) NOT NULL COLLATE BINARY, publication_date DATE NOT NULL, description VARCHAR(255) NOT NULL, CONSTRAINT FK_4A1B2A92F675F31B FOREIGN KEY (author_id) REFERENCES author (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_4A1B2A92C54C8C93 FOREIGN KEY (type_id) REFERENCES type_book (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_4A1B2A92D4C006C8 FOREIGN KEY (borrow_id) REFERENCES borrow (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO books (id, author_id, type_id, borrow_id, title, cover, publication_date) SELECT id, author_id, type_id, borrow_id, title, cover, publication_date FROM __temp__books');
